...
- Installer: the migration runs by default — the prompt defaults to yes, `--no-interaction` answers yes, an existing table skips the step (`migrate --force`, so it also works with `APP_ENV=production`). A declined migration makes the install refuse the import and exit non-zero instead of failing on a missing table.
- Addresses table: "Import from swisstopo" header button. Dispatches `Jobs\ImportRegister` (queued; inline on the sync driver), respects the unchanged short-circuit and tells the clicker either way with a database notification (result, "register unchanged" or the failure text); the button is disabled and reads "Import running…" while a run holds the lock (table polls until it is free). Authorization: `AddressPolicy::import()` when the app has one, otherwise allowed; the resource declares the Filament Shield prefixes `view_any`, `view`, `import` (Shield's `HasShieldPermissions` interface is shimmed when Shield is absent).
- Installer: the suggested time for the nightly import is no longer 03:00 for everyone but a fixed minute between 03:00 and 04:59 hashed from the app name and the plugin name (`ScheduleInstaller::defaultTime()`), so the verwaltis do not all hit swisstopo at once. The literal is written into `routes/console.php` as before.
